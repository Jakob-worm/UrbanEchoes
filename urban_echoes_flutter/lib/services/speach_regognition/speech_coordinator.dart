...
class SpeechCoordinator extends ChangeNotifier {
  ...
  String? get errorMessage =>
      _speechService.errorMessage ?? _birdService.errorMessage;
  ...
  Future<void> handleConfirmationResponse(bool confirmed) async {
    ...
    if (confirmed) {
      ...
      _audioService.playPrompt('systemet_er_i_tvil');
      _logDebug(
          'SystemInDoubt state activated with ${_possibleBirds.length} alternatives');
    }
  }
  ...
  void _onWordUpdate() {
    ...
    _logDebug(
        'Processing confirmation word: ${_wordService.recognizedSpecialWord}');
  ...
  void _processConfirmationSpeech(String recognizedText) {
    _logDebug(
        'In confirmation state, checking text for confirmation: $recognizedText');
  ...
  void _directHandleSystemInDoubt(String recognizedText) {
    if (_possibleBirds.isEmpty) {
      _logDebug(
          'WARNING: In system in doubt state but no possible birds available');
      return;
    }
    ...
    if (_directCheckForPositiveResponse(lowerText) &&
        _possibleBirds.isNotEmpty) {
      _logDebug(
          'Detected positive response in doubt state - selecting first option');
      if (_speechService.isListening) {
        _speechService.stopListening();
      }
  ...
  void _processWordResponseInDoubtState(String word) {
    if (_isPositiveResponse(word) || _isNegativeResponse(word)) {
      ...
      if (_isPositiveResponse(word) && _possibleBirds.isNotEmpty) {
        _logDebug(
            'Positive response in doubt state - selecting first option: ${_possibleBirds[0]}');
        handleBirdSelection(_possibleBirds[0]);
      } else if (_isNegativeResponse(word)) {
  ...
  void _findAlternativeBirds([String? forcedOriginalBird]) {
    ...
    _logDebug(
        'Processing negative response - preparing to show alternatives for: $birdToUse');
    ...
    if (_birdService.possibleMatches.length > 1) {
      _possibleBirds = _birdService.possibleMatches
          ...
          .toList();
      _logDebug(
          'Using existing matches sorted by confidence: ${_possibleBirds.join(", ")}');
    } else {
      ...
      if (_birdService.possibleMatches.length > 1) {
        ...
        if (_directCheckForNegativeResponse(textToProcess.toLowerCase())) {
          ...
          _logDebug(
              'Text contains negative response, using original bird name instead: $birdToUse');
        }
        ...
        _possibleBirds = _birdService.possibleMatches
            ...
            .toList();
        _logDebug(
            'Using text processing matches: ${_possibleBirds.join(", ")}');
      }
    }
    ...
    _logDebug(
        'Final possible birds by confidence: ${_possibleBirds.join(", ")}');
  }
  ...
  void _addFallbackBirds([String? birdToExclude]) {
    ...
    _logDebug(
        'Adding fallback birds, current count: ${_possibleBirds.length}, excluding: $excludeBird');
    ...
    if (_possibleBirds.isEmpty) {
      ...
      if (allBirds.isEmpty) {
        _logDebug(
            'WARNING: birdService.birdNames is empty! Using emergency fallback list');
  ...
  void _startConfirmationTimeout() {
    _confirmationTimer?.cancel();
    _confirmationTimer =
        Timer(Duration(seconds: _confirmationTimeoutSeconds), () {
      if (_currentState == RecognitionState.waitingForConfirmation ||
          _currentState == RecognitionState.systemInDoubt) {
  ...
  Future<BirdObservation?> _createAndUploadObservation() async {
    ...
    if (createdObservation != null) {
      _logDebug(
          'Successfully saved and uploaded observation for $_currentBirdInQuestion');
  ...
  List<String> _getFallbackBirdsBasedOnPhonetics(String birdName) {
    ...
    if (allBirds.isEmpty) {
      _logDebug(
          'WARNING: No birds in birdService.birdNames, using emergency list');
  ...
  bool _isDismissalCommand(String text) {
    return text.contains('ingen') ||
        text.contains('none') ||
        text.contains('annuller') ||
  ...
  void _debugCurrentState() {
    ...
    _logDebug(
        'WordService special word: ${_wordService.recognizedSpecialWord}');
    _logDebug('BirdService matched bird: ${_birdService.matchedBird}');
    _logDebug(
        'BirdService possible matches: ${_birdService.possibleMatches.join(", ")}');
